                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('npk') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="npk" id="npk" value="{{ old('npk', $user->npk) }}" required>
                            @if($errors->has(''))
                        ...
                        <div class="form-group {{ $errors->has('job') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="job_id" id="job_id">
                                @foreach($jobs as $id => $job)
                                    <option value="{{ $id }}" {{ ($user->job ? $user->job->id : old('job_id')) == $id ? 'selected' : '' }}>{{ $job }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('dept') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="dept_id" id="dept_id" required>
                                @foreach($depts as $id => $dept)
                                    <option value="{{ $id }}" {{ ($user->dept ? $user->dept->id : old('dept_id')) == $id ? 'selected' : '' }}>{{ $dept }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('parent') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="parent_id" id="parent_id" required>
                                @foreach($parents as $id => $parent)
                                    <option value="{{ $id }}" {{ ($user->parent ? $user->parent->id : old('parent_id')) == $id ? 'selected' : '' }}>{{ $parent }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
