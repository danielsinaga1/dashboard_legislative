        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.incidentReport.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.incident-reports.update", [$incidentReport->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        
                        <div class="form-group {{ $errors->has('no_laporan') ? 'has-error' : '' }}">
                            <label class="required" for="no_laporan">{{ trans('cruds.incidentReport.fields.no_laporan') }}</label>
                            <input class="form-control" type="text" name="no_laporan" id="no_laporan" value="{{ old('no_laporan', $incidentReport->no_laporan) }}" readonly>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.incidentReport.fields.no_laporan_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('root_cause') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="root_cause_id" id="root_cause_id" required>
                                @foreach($root_causes as $id => $root_cause)
                                    <option value="{{ $id }}" {{ ($incidentReport->root_cause ? $incidentReport->root_cause->id : old('root_cause_id')) == $id ? 'selected' : '' }}>{{ $root_cause }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id">
                                @foreach($categories as $id => $category)
                                    <option value="{{ $id }}" {{ ($incidentReport->category ? $incidentReport->category->id : old('category_id')) == $id ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('classify') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="classify_id" id="classify_id">
                                @foreach($classifies as $id => $classify)
                                    <option value="{{ $id }}" {{ ($incidentReport->classify ? $incidentReport->classify->id : old('classify_id')) == $id ? 'selected' : '' }}>{{ $classify }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group {{ $errors->has('perbaikan') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="perbaikan" id="perbaikan" value="{{ old('perbaikan', $incidentReport->perbaikan) }}">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('pencegahan') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="pencegahan" id="pencegahan" value="{{ old('pencegahan', $incidentReport->pencegahan) }}">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('photos') ? 'has-error' : '' }}">
                            <label for="photos">{{ trans('cruds.incidentReport.fields.photos') }} <h5 style="color:red;">(Size Max. 3 MB)</h5></label>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $incidentReport->description) }}" required>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                        ...
                        <div class="form-group {{ $errors->has('dept_designated') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="dept_designated_id" id="dept_designated_id">
                                @foreach($dept_designateds as $id => $dept_designated)
                                    <option value="{{ $id }}" {{ ($incidentReport->dept_designated ? $incidentReport->dept_designated->id : old('dept_designated_id')) == $id ? 'selected' : '' }}>{{ $dept_designated }}</option>
                                @endforeach
                            </select>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                        ...
                        <div class="form-group">
                            <label class="required" for="direct_cost">{{ trans('cruds.incidentReport.fields.direct_cost') }}</label>
                            <input class="form-control" type="text" name="direct_cost" id="direct_cost" value="{{ old('direct_cost', $incidentReport->direct_cost) }}">
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.incidentReport.fields.direct_cost_helper') }}</span>
                        </div>

                        <div class="form-group">
                            <label class="required" for="copq">{{ trans('cruds.incidentReport.fields.copq') }}</label>
                            <input class="form-control" type="text" name="copq" id="copq" value="{{ old('copq', $incidentReport->copq) }}" readonly>
                            @if($errors->has(''))
                                <span class="help-block" role="alert">{{ $errors->first('') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.incidentReport.fields.copq_helper') }}</span>
                        </div>
